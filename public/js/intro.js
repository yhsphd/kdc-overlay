let intro_countdownElement =
  document.getElementsByClassName("intro-countdown")[0];
let intro_scheduleElement =
  document.getElementsByClassName("intro-schedule")[0];
...
function intro_updateCountdown() {
  if (schedule >= new Date()) {
    ...
    intro_countdownElement.innerText = secondsToMMSS(
...
function intro_updateSchedule(inputSchedule) {
  ...
  intro_scheduleElement.innerText = `${schedule.getFullYear()}.${(
    schedule.getMonth() + 1
  )
    .toString()
    .padStart(2, "0")}.${schedule
    .getDate()
    .toString()
    .padStart(2, "0")}. ${schedule
    .getHours()
