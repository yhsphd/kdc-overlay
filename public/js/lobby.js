let lobby_acronymElements = document
  .getElementById("topbar")
  .getElementsByClassName("acronym");
...
let lobby_scoreElements = document.getElementsByClassName(
  "scoreBox-teamBox-scoreSum"
);
let lobby_accElements = document.getElementsByClassName(
  "scoreBox-teamBox-accAvg"
);
...
function lobby_updateScores(lobby) {
  ...
  lobby_accElements[0].innerText =
    ((lobby.players[0].acc + lobby.players[1].acc) / 2).toFixed(2) + "%";
  lobby_accElements[1].innerText =
    ((lobby.players[2].acc + lobby.players[3].acc) / 2).toFixed(2) + "%";
...
function lobby_updateSetScores(lobby) {
  if (bo !== lobby.bo) {
    ...
    for (let i = 0; i < (bo + 1) / 2; i++) {
      lobby_setScoreBoxElements[0].appendChild(
        setScorePointBaseElement.cloneNode(true)
      );
      lobby_setScoreBoxElements[1].appendChild(
        setScorePointBaseElement.cloneNode(true)
      );
    }
  } else {
    for (let i = 0; i < 2; i++) {
      if (setScores[i] < lobby.set_scores[i]) {
        ...
        setScores[i]++;
        lobby_setScoreBoxElements[i].getElementsByClassName("setScorePoint")[
          setScores[i] - 1
        ].style.backgroundColor = "#E7B942";
...
function lobby_updateVisibilities(overlayData) {
  if (tempState !== overlayData.progress.state) {
    if (overlayData.progress.state === 1) {
      // idle
      lobby_showChat();
      mapCompact_showMapCompacts();
    }

    if (overlayData.progress.state === 3) {
      // playing
      lobby_showScores();
      lobby_showLeaderboard();
    }

    if (overlayData.progress.state === 4 && tempState === 3) {
      // playing -> result
