...
function lobby_leaderboard_getDOM() {
  try {
    lobby_leaderboardBoxElements = document.getElementsByClassName(
      "lobby-leaderboardBox"
    );
    lobby_leaderboardBox_rankElements = document.getElementsByClassName(
      "lobby-leaderboardBox-rank"
    );
    lobby_leaderboardBox_pfpElements = document.getElementsByClassName(
      "lobby-leaderboardBox-pfp"
    );
    lobby_leaderboardBox_nickElements = document.getElementsByClassName(
      "lobby-leaderboardBox-nick"
    );
    lobby_leaderboardBox_scoreElements = document.getElementsByClassName(
      "lobby-leaderboardBox-score"
    );
    lobby_leaderboardBox_comboElements = document.getElementsByClassName(
      "lobby-leaderboardBox-combo"
    );
    lobby_leaderboardBox_accElements = document.getElementsByClassName(
      "lobby-leaderboardBox-acc"
    );
...
function lobby_showLeaderboard(hide = false) {
  if (!hide) {
    ...
    setTimeout(() => {
      for (let i = 0; i < lobby_leaderboardBoxElements.length; i++) {
        setTimeout(() => {
          ...
          lobby_leaderboardBoxElements[i].addEventListener(
            "animationend",
            () => {
              lobby_leaderboardBoxElements[i].style.transition =
                "transform 500ms cubic-bezier(0, 1.4, 0.6, 1)";
            },
            { once: true }
          );
        }, 100 * i);
    ...
    for (let i = 0; i < playerOrder.length; i++) {
      setTimeout(() => {
        lobby_leaderboardBoxElements[playerOrder[i]].style.transition =
          "transform 500ms cubic-bezier(0, 0, 0, 1)";
        lobby_leaderboardBoxElements[playerOrder[i]].style.transform +=
          " translateX(-100%)";
        lobby_leaderboardBoxElements[playerOrder[i]].addEventListener(
          "animationend",
          () => {
            lobby_leaderboardBoxElements[playerOrder[i]].style.transform =
              "translateX(-100%)";
...
function updateLeaderboardValue(players) {
  for (let i = 0; i < lobby_leaderboardBoxElements.length; i++) {
    lobby_leaderboardBox_pfpElements[i].src = `https://a.ppy.sh/${
      players[i].id === 0 ? 1 : players[i].id
    }`;
    lobby_leaderboardBox_nickElements[i].innerText = players[i].nick;
    lobby_leaderboardBox_scoreElements[i].innerText = numberWithCommas(
      players[i].score
    );
    lobby_leaderboardBox_comboElements[i].innerText = `${players[i].combo}x`;
    lobby_leaderboardBox_accElements[i].innerText = `${players[i].acc.toFixed(
      2
    )}%`;
  }
}
...
function updateLeaderboardOrder(players) {
  ...
  for (let i = 0; i < lobby_leaderboardBoxElements.length; i++) {
    lobby_leaderboardBoxElements[
      i
    ].style.transform = `translateY(${transformsY[i]}px)`;
