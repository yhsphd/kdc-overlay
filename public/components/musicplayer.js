...
function musicplayer_getDOM() {
  try {
    musicplayer_coverElement = document.getElementsByClassName(
      "intro-musicplayer-cover"
    )[0];
    musicplayer_titleElement = document
      .getElementsByClassName("intro-musicplayer-title")[0]
      .querySelectorAll("span")[0];
    musicplayer_artistElement = document
      .getElementsByClassName("intro-musicplayer-artist")[0]
      .querySelectorAll("span")[0];
    musicplayer_progressbarElement = document.getElementsByClassName(
      "intro-musicplayer-progressbar"
    )[0];
    musicplayer_positionElement = document.getElementsByClassName(
      "intro-musicplayer-position"
    )[0];
    musicplayer_lengthElement = document.getElementsByClassName(
      "intro-musicplayer-length"
    )[0];
  } catch (e) {
  ...
  if (
    !musicplayer_coverElement ||
    !musicplayer_titleElement ||
    !musicplayer_artistElement ||
    !musicplayer_progressbarElement ||
    !musicplayer_positionElement ||
    !musicplayer_lengthElement
  ) {
    setTimeout(musicplayer_getDOM, 1000);
  }
}
...
function updateMusicplayer(nowPlaying) {
  ...
  if (nowPlaying.mode === "fb2k") {
    ...
    musicplayer_progressbarElement.style.width =
      (npInfo.time / npInfo.length) * 100 + "%";
  } else {
    musicplayer_positionElement.innerText = secondsToMMSS(npInfo.time / 1000);
    musicplayer_lengthElement.innerText = secondsToMMSS(
      npInfo.stats.length / 1000
    );
    musicplayer_progressbarElement.style.width =
      (npInfo.time / npInfo.stats.length) * 100 + "%";
  }

  if (tempTitle !== npInfo.title || tempArtist !== npInfo.artist) {
    // song changed
    if (nowPlaying.mode === "fb2k") {
      musicplayer_coverElement.style.backgroundImage = `url("${
        npInfo.cover + "?" + new Date().getTime()
      }")`;
    } else {
