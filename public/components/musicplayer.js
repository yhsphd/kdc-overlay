...
function musicplayer_getDOM() {
    try {
        ...
        musicplayer_titleElement = document.getElementsByClassName("intro-musicplayer-title")[0].querySelectorAll("span")[0];
        musicplayer_artistElement = document.getElementsByClassName("intro-musicplayer-artist")[0].querySelectorAll("span")[0];
...
function updateMusicplayer(nowPlaying) {
    ...
    if (tempTitle !== npInfo.title || tempArtist !== npInfo.artist) {       // song changed
        if (nowPlaying.mode === "fb2k") {
